...
@section('title', $title ?? 'NIN Modification')
@section('content')
    <div class="page">
                                    ...
                                    <tbody>
                                        @forelse ($crmSubmissions as $submission)
                                            <tr>
                                                ...
                                                <td><small class="text-muted">{{ optional($submission->submission_date)->format('M d, Y') ?? 'N/A' }}</small></td>
                                                <td>
                                                    @php
                                                        $fileUrl = '';
...
@push('page-js')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const proceedBtn = document.getElementById('proceed-attestation-btn');
            const dobInputs = document.querySelectorAll('.dob-input');

            // Price & DOB Wizard toggle logic
            if (serviceSelect) {
                serviceSelect.addEventListener('change', function() {
                    const selectedOption = this.options[this.selectedIndex];
                    const price = selectedOption.getAttribute('data-price');
                    const description = selectedOption.getAttribute('data-description');
                    const code = selectedOption.getAttribute('data-field-code');
                    ...
                    // Check if it is a DOB update (by checking code 035/036 or name text)
                    const isDob = code === '035' || code === '036' || text.includes('date of birth') || text.includes('dob');

                    // Reset wizard visibility & input states
                    dobWizard.classList.add('d-none');
                    dobInputs.forEach(input => input.disabled = true);
                ...
                // Trigger change event once to set initial price/display if any old/preselected option
                if (serviceSelect.value) {
                    serviceSelect.dispatchEvent(new Event('change'));
                }
            }

            // Proceed to Attestation Wizard
            if (proceedBtn) {
                proceedBtn.addEventListener('click', function() {
                    dobProceedBtn.classList.add('d-none');
                    dobWizard.classList.remove('d-none');
                    
                    // Enable inputs for form submission
                    dobInputs.forEach(input => input.disabled = false);
            ...
            function showStep(stepNum) {
                ...
                if (progressBar) {
                    progressBar.style.width = ((stepNum / 8) * 100) + '%';
                }

                // Update badge
                const badge = document.getElementById('step-badge');
                if (badge) {
                    badge.textContent = `Step ${stepNum} of 8`;
                }
            }
            ...
            document.querySelectorAll('.next-step').forEach(btn => {
                btn.addEventListener('click', function() {
                    const nextStepId = this.getAttribute('data-next'); // e.g. "step-2"
            ...
            document.querySelectorAll('.prev-step').forEach(btn => {
                btn.addEventListener('click', function() {
                    const prevStepId = this.getAttribute('data-prev'); // e.g. "step-1"
