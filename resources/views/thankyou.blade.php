...
@section('title', 'Thank You - Transaction Successful')
    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            @php
                ...
                $pin = $meta['pin'] ?? session('pin') ?? null;
                $serial = $meta['serial'] ?? session('serial') ?? null;
            ...
            <div class="receipt-card" id="receipt">
                <div class="receipt-header" style="background: linear-gradient(135deg, #114b5f 0%, #28a745 100%);">
                    <div class="header-decoration"></div>
                    <div class="receipt-header-content">
                        <div class="status-badge" style="background: rgba(255, 255, 255, 0.25);">
                ...
                <div class="receipt-body">
                    @if(session('success'))
                        <div class="alert alert-success text-center mb-4" style="border-radius: 12px; background-color: #d4edda; border-color: #c3e6cb; color: #155724; font-weight: 500; font-size: 15px; padding: 15px;">
                            <i class="fas fa-check-double me-2"></i> {!! session('success') !!}
                        </div>
                    @endif

                    <h2 class="section-title">Transaction Receipt</h2>
                    ...
                    try {
                        ...
                        if (navigator.share && navigator.canShare && navigator.canShare({ files: [new File([], '')] })) {
                            ...
                            if (navigator.clipboard) {
                                const shareText = `ZEPA Transaction Receipt\nReference ID: ${transactionId}\nService: ${document.querySelector('.receipt-body .info-row:nth-child(2) .info-value')?.textContent?.trim() || 'N/A'}\nAmount: ${document.querySelector('.receipt-body .info-row:nth-child(4) .info-value')?.textContent?.trim() || 'N/A'}\nStatus: Successful`;
                                await navigator.clipboard.writeText(shareText);
                                alert('Direct image sharing is not supported by your browser/device. The transaction details have been copied to your clipboard!');
                            } else {
