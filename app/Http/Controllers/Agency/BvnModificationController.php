...
class BvnModificationController extends Controller
{
    public function index(Request $request)
    {
        ...
        $role = $user->role ?? 'user';
        $affidavitField = ServiceField::where('field_code', '029')->first();
        $affidavitPrice = 0;
        if ($affidavitField) {
    ...
    public function store(Request $request)
    {
        ...
        $affidavitField = ServiceField::where('field_code', '029')->firstOrFail();
        ...
        if ($wallet->balance < $totalAmount) {
            ...
            return redirect()->route('bvn-modification')->withErrors(['wallet' => $msg])->withInput();
        }
        ...
        try {
            ...
            return redirect()->route('bvn-modification')->with([
                'status' => 'success',
                'message' => $msg,
            ]);
        } catch (\Exception $e) {
            ...
            return redirect()->route('bvn-modification')->withErrors([
    ...
    public function getFields($serviceId)
    {
        ...
        $service = Service::with(['fields' => function ($query) {
            $query->where('is_active', 1);
        }])->findOrFail($serviceId);

        $fields = $service->fields->map(function ($field) use ($role) {
            return [
                'id' => $field->id,
                'field_name' => $field->field_name,
