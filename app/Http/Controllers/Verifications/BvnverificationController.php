...
class BvnverificationController extends Controller
{
     ...
     */
    private function processSuccessTransaction($servicePrice, $user, $serviceField, $service, $bvnData)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            Verification::create([
                ...
                // BVN-specific extra fields
                'self_origin_state' => $apiData['stateOfOrigin'] ?? ($apiData['state_of_origin'] ?? null),
                'self_origin_lga'   => $apiData['lgaOfOrigin'] ?? ($apiData['lga_of_origin'] ?? null),
                'maritalstatus'     => $apiData['maritalStatus'] ?? ($apiData['marital_status'] ?? null),
                'registrationDate'  => $apiData['registrationDate'] ?? ($apiData['registration_date'] ?? null),
                'enrollmentBank'    => $apiData['enrollmentBank'] ?? ($apiData['enrollment_bank'] ?? null),
                'enrollmentBranch'  => $apiData['enrollmentBranch'] ?? ($apiData['enrollment_branch'] ?? null),
                'watchListed'       => $apiData['watchListed'] ?? ($apiData['watch_listed'] ?? null),
                'levelOfAccount'    => $apiData['levelOfAccount'] ?? ($apiData['level_of_account'] ?? null),
                'residence_state'   => $apiData['stateOfResidence'] ?? ($apiData['state_of_residence'] ?? null),
                'residence_lga'     => $apiData['lgaOfResidence'] ?? ($apiData['lga_of_residence'] ?? null),
                'residence_address' => $apiData['residentialAddress'] ?? ($apiData['residential_address'] ?? null),
                'nationality'       => $apiData['nationality'] ?? null,
                'nameOnCard'        => $apiData['nameOnCard'] ?? ($apiData['name_on_card'] ?? null),
                'phoneNumber2'      => $apiData['phoneNumber2'] ?? ($apiData['phone_number2'] ?? ($apiData['phone2'] ?? null)),
            ]);
