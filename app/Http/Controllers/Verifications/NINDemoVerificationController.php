...
class NINDemoVerificationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!$isSuccessful && isset($data['message'])) {
            ...
            $isRequestError = true;
            $errorMessage = 'Verification request failed. Please try again later.';
        } catch (\Exception $e) {
            ...
            $errorMessage = 'A system error occurred. Please contact support if this persists.';
        }
        ...
        // Log API errors for debugging if not connection/request/system error
        if (!$isConnectionError && !$isRequestError) {
     ...
     */
    private function processSuccessTransaction($servicePrice, $user, $serviceField, $service, $apiResponse)
    {
        DB::beginTransaction();
