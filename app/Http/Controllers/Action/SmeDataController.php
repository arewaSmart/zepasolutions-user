...
class SmeDataController extends Controller
{
    ...
    public function buySMEdata(Request $request)
    {
        ...
        // Check Wallet Balance & pre-debit
        try {
        ...
        $isSuccessful = false;
        $data = null;
        $errorMessage = 'Data purchase failed. Please try again.';
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                $errorMessage = 'Data provider returned an error code: ' . $response->status();
            }

        } catch (\Exception $e) {
            Log::error('SME Data API Connection Error: ' . $e->getMessage());
            $errorMessage = 'Could not connect to data provider. Please try again later.';
        }
